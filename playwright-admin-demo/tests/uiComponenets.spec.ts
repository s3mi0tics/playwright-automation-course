...
test.describe("Form layouts page", () => {
  ...
  test("radio buttons", async ({ page }) => {
    ...
    expect(
      await usingTheGridForm.getByLabel("Option 1").isChecked(),
    ).toBeFalsy();
  });
});
...
test("checkboxes", async ({ page }) => {
  ...
  // .check() will check the checkbox without clicking it, if it is already checked it will not uncheck it, if it is unchecked it will check it
  await checkbox1.check({ force: true });
  expect(await checkbox1.isChecked()).toBeTruthy();
  //   .uncheck() will uncheck the checkbox without clicking it, if it is already unchecked it will not check it, if it is checked it will uncheck it
  await checkbox1.uncheck({ force: true });
  expect(await checkbox1.isChecked()).toBeFalsy();
  //   .click() will actually click the checkbox and if it is already checked it will uncheck it, if it is unchecked it will check it (don't know what to expect here because we dont know the initial state of the checkbox)
